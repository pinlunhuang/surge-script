...
const lk = new ToolKit(`Bing積分`, `BingPoint`, {"httpApi": "ffff@10.0.0.19:6166"})
		...
		lk.boxJsJsonBuilder({
			...
			"settings": [
				...
				},
				{
					...
					"name": "Bing每日搜索cookie(移動端)",
					"val": "",
					"type": "text",
					"desc": "請使用手機打開https://tw.bing.com/search?q=test抓取對應請求的cookie"
				},
				{
					"id": searchMobileAmountKey,
					"name": "Bing每日執行搜索(移動端)次數",
					"val": 10,
					"type": "number",
					"desc": "Bing每日執行搜索(移動端)次數"
				},
				{
					...
					"val": "",
					"type": "text",
					"desc": "請使用電腦打開https://tw.bing.com/search?q=test抓取對應請求的cookie"
				},
				{
					"id": searchPcAmountKey,
					"name": "Bing每日執行搜索(PC)次数",
					"val": 10,
					"type": "number",
					"desc": "Bing每日執行搜索(PC)次數"
				},
				{
					"id": searchEdgeAmountKey,
					"name": "Bing每日執行搜索(Edge)次數",
					"val": 10,
					"type": "number",
					"desc": "Bing每日執行搜索(Edge)次數"
				}
			],
	...
	if (lk.isGetCookie(/\/rewards\.bing\.com/)) {
		...
		try {
			...
			if (!!bingHeader) {
				...
				lk.appendNotifyInfo('🎉成功獲取cookie，可以關閉相應腳本')
			}
		} catch (e) {
			lk.execFail()
			lk.appendNotifyInfo('❌獲取bing cookie失敗')
		}
	}
	lk.msg(``)
	lk.done()
...
async function dealMsg(dashBoard, newPoint) {
	return new Promise((resolve, _reject) => {
		...
		if (availablePoints != "-" && cachePoint) {
			...
			lk.prependNotifyInfo(`本次執行：${increaseAmount >= 0 ? "+" + increaseAmount : increaseAmount}`)
		}
		resolve(`現有積分：${availablePoints}${newPoint > 0 ? "+" + newPoint : ""}   日常獲得：${dashBoard?.dashboard?.userStatus?.counters?.dailyPoint[0]?.pointProgress || "-"}/${dashBoard?.dashboard?.userStatus?.counters?.dailyPoint[0]?.pointProgressMax || "-"}`)
	})
}
...
async function all() {
	...
	if (bingPointCookie == '') {
		lk.execFail()
		lk.appendNotifyInfo(`⚠️請先打開rewards.bing.com獲取cookie`)
	} else {
		bingPointHeader = {}
...
function doReportActForQuiz(title, item, rvt) {
	return new Promise((resolve, _reject) => {
		// todo 預留方法，目前官網手動都做不了的任務🤣
		const t = '做問答獎勵任務：' + title
		lk.log(t)
		let ret = 0
		let url = {
		...
		lk.post(url, (error, _response, data) => {
			try {
				if (error) {
					lk.execFail()
					lk.log(error)
					lk.appendNotifyInfo(`❌${t}失敗，請稍後再試`)
				} else {
				...
				lk.logErr(e)
				lk.log(`bing返回數據：${data}`)
				lk.execFail()
				lk.appendNotifyInfo(`❌${t}錯誤，請稍後再試`)
			} finally {
				resolve(ret)
			}
		})
...
function searchEdge() {
	return new Promise(async (resolve, _reject) => {
		...
		if (nowString != isSearchEdgeRepeat || searchEdgeCount < searchEdgeAmount) {
			...
			try {
				if (!isAlwaysSearch) {
					lk.log(`保存今天(${nowString})搜索(Edge)次數：${searchEdgeCount}`)
...
function searchMobile() {
	return new Promise(async (resolve, _reject) => {
		...
		if (!isAlwaysSearch && nowString == isSearchMobileRepeat && searchMobileCount >= searchMobileAmount) {
			lk.log(`今日搜索(移動端)已達配置上限：${searchMobileAmount}次`)
		...
		if (nowString != isSearchMobileRepeat || searchMobileCount < searchMobileAmount) {
			...
			try {
				if (!isAlwaysSearch) {
					lk.log(`保存今天(${nowString})搜索(移動端)次數：${searchMobileCount}`)
...
function searchPc() {
	return new Promise(async (resolve, _reject) => {
		...
		if (!isAlwaysSearch && nowString == isSearchRepeat && searchPcCount >= searchPcAmount) {
			lk.log(`今日搜索(PC)已達配置上限：${searchPcAmount}次`)
		...
		if (nowString != isSearchRepeat || searchPcCount < searchPcAmount) {
			...
			try {
				if (!isAlwaysSearch) {
					lk.log(`保存今天(${nowString})搜索(PC)次數：${searchPcCount}`)
		...
		if (morePromotions.length > 0) {
			...
			while (true) {
				...
				if (todoCount + completeCount >= morePromotions.length) {
					lk.log(`任務都做完了，退出`)
					err = `🎉任務都做完啦，共獲得${completePoint}積分`
					break
				}
				if (new Date().getTime() - lk.startTime > 5 * 1000) {
					lk.log(`執行超時，強制退出`)
					err = "❌執行超時，強制退出（請添加分流切換節點）"
					break
				}
				await lk.sleep(100)
			...
			if (!err) {
				if (totalCount > 0) {
					lk.execFail()
					lk.prependNotifyInfo(`🎉成功：${sucCount}個，❌失敗：${failCount}個`)
				} else {
					lk.appendNotifyInfo(`🎉今天的任務都做完啦`)
				}
			} else {
				lk.prependNotifyInfo(err)
				lk.prependNotifyInfo(`🎉：${sucCount}個，❌：${failCount}個，今日已完成：${completeCount}個`)
			}
			resolve(newPoint)
		} else {
			lk.execFail()
			lk.prependNotifyInfo(`❌未獲取到活動訊息`)
			resolve(newPoint)
		}
	})
}
